...
QQ 授权码和签名密钥存 macOS Keychain，普通 TOML 配置只保存账户与选项；环境变量只临时覆盖非敏感配置。发件人规范化后精确匹配白名单。邮件头线程引用、短任务 ID、带签名令牌必须一致；缺失或冲突则拒绝，不能降级成只凭主题中的任务 ID 执行。默认 72 小时有效（L1 之后由七天改为三天，见 plans/phase-04.md 的 D4），可配置，任务关闭立即撤销。

签名用于完整性验证，不等于邮件内容加密；令牌绑定任务、用户、有效期、通知 ID。令牌载体已用真实 QQ 客户端验证（L1，2026-09-20）：正文页脚在不带引用原文的回复中会丢失，而主题标签在测过的客户端中都原样保留（自定义头不会被回复带回，这一条是先验判断，L1 未实测）。因此令牌放在主题标签 `[TC <任务 ID> <令牌>]` 中，**主题是唯一的验证输入**；正文页脚仍放一份，但只是给人看的副本，验证时永不回读——页脚在回复里一定落在被剥离的引用区，多轮之后那里会堆着历次的旧令牌。代价是令牌离开了正文：它出现在锁屏与通知预览、邮件列表、服务端的主题索引与搜索、客户端的本地全文索引里，还会随转发、退信、自动回复和用户自己写的回复沿线程传播；这些都不受本地的加密与安全删除约束。凭有效期、任务关闭即撤销和专用机器人邮箱来限制。线程引用以 QQ 实际投递的 Message-ID 为准，QQ 会改写我方 ID。发件人头可以伪造，不能单独作为身份凭据；QQ 同域投递不带 `Authentication-Results` 与 `Received`，所以也无法靠它们判断来源。令牌属于持有者凭证，不可出现在公开日志中。

Message-ID 去重之外，收取记录还需要邮箱账户、UIDVALIDITY、UID 和正文摘要。SQLite 唯一约束与入队必须同一事务。相同 Message-ID 但正文冲突应拒绝并告警。邮件自动回复与退信不可触发 Agent：令牌在主题中，白名单地址的假期自动回复会原样带回主题标签，因此这项过滤排在一切标签与令牌校验之前。过滤是启发式的——L1 实测 QQ 的假期自动回复不带任何头部信号，只能按主题前缀识别——另有与分类无关的回环刹车兜底；接收通知的邮箱不要开启假期自动回复。忙碌任务的 FIFO 顺序以本地入队序号定义，不能信任发件人提供的 Date。

默认仅保留元数据历史，但未投递回复、待发邮件在队列中必须暂存正文，否则无法恢复。待处理载荷加密落盘，密钥存 Keychain，完成后清理；用户可主动开启历史记录。此为“元数据默认”的运行队列例外，必须向用户说明。

Keychain 的威胁模型（Phase 4a 已实现，alpha 接受）：条目经 `/usr/bin/security` 创建，受信任应用因此是 `/usr/bin/security`、分区为 `apple-tool:`。同一用户的任何进程——包括 Agent 执行的 shell 命令——都能静默读出 QQ 授权码与两把密钥。这不是推断：L1 第 5 步用合成条目实测，Claude Code 与 Codex 的 shell 都读出了机密数据，无授权弹窗、无拒绝。读出之后，它可以用令牌签名密钥签发有效令牌，伪造出通过发件人白名单、线程引用、主题标签与令牌全部校验的回复，把任意内容注入任一任务，也可以直接改写本地队列；TurnCourier 不防同一用户下的进程。Keychain 在这里防的是：明文进入配置、仓库、日志与备份；其他系统用户读取；在没有登录密码的情况下离线访问磁盘。不改用进程内调用 Security.framework，是因为 Go 构建的二进制只有随构建变化的临时签名或完全没有签名，把访问权限绑定到 TurnCourier 自身会使每次升级都重新弹窗授权，后台进程被卡住。缓解措施：使用专用机器人邮箱而不是个人邮箱，授权码可随时在 QQ 邮箱中停用。发布阶段再评估 Developer ID 签名加进程内调用，届时优先选纯 Go 的 purego，不引入 CGO。

正文摘要与密文（Phase 4a 已实现）：`inbound_messages.body_sha256` 的列名因 `0001` 已发布而保留，但自迁移 `0002` 起保存的是键控摘要 HMAC-SHA256（令牌签名密钥、独立前缀、对解析器输出的新正文计算），不是裸摘要。待处理正文以 AES-256-GCM 加密后落盘，关联数据绑定用途、密钥号、任务 ID 与序号，密文被调换到别的行时无法解密；进入终态（通知 SENT 或 ABANDONED、回复 ACKNOWLEDGED 或 REJECTED）时由数据库触发器在同一事务中删除。数据库启用 `secure_delete`，删除后尽力执行 `PRAGMA wal_checkpoint(TRUNCATE)`，打开数据库时也执行一次。APFS 快照与 Time Machine 备份中的残留不在 SQLite 能控制的范围内：正文加密密钥仍在 Keychain 中时，这些残留的密文可以被解密。把数据目录恢复到旧备份会让已发出的通知重发、已确认的回复再次派发。

Agent 接收与 SQLite 提交无法天然形成一个事务。发生“已投递但确认丢失”时进入待核对状态；未证明 Agent 端幂等前禁止盲目重发，不承诺端到端 exactly-once。SMTP 也可能出现服务端接收但本地未获确认，保留重复通知可能性。L1 证明 QQ 会改写发出邮件的 Message-ID，我方 ID 只在 `X-OQ-MSGID` 中保留，因此我方的稳定 Message-ID 不能指望在收件人一侧起到去重作用；线程绑定以 QQ 实际投递的 ID 为准。
...
开发顺序：竞品与接口研究 → 公开工程骨架 → 配置/存储/状态机 → 邮件闭环 → Codex → Claude → 安全与恢复 → 真实验收 → 发布。当前处于邮件闭环阶段：离线部分（4a）已实现并通过测试，L1 真机探测已完成，4b 的任务契约待维护者确认后实现。
