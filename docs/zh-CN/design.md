...
QQ 授权码和签名密钥存 macOS Keychain，普通 TOML 配置只保存账户与选项；环境变量只临时覆盖非敏感配置。发件人规范化后精确匹配白名单。邮件头线程引用、短任务 ID、带签名令牌必须一致；缺失或冲突则拒绝，不能降级成只凭主题中的任务 ID 执行。默认七天有效，可配置，任务关闭立即撤销。
