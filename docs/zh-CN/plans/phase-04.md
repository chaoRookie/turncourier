...
> **状态：决策稿。** 本文件先列出开工前须由维护者确认的决策、调研证据、范围与任务概要。第 0 节确认后，再按 Phase 3 的格式补齐每个任务的契约、测试清单与提交信息，然后才开始实现。执行方式沿用 Phase 3：每个任务由独立子代理实现，经规格与质量审查（含变异测试）、修复和独立复查后提交，阶段末做整阶段审查。
