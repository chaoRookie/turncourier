...
**审查后的修正（2026-09-24）：** 规格与质量两名审查子代理各自复核并做变异测试，结论都是「修复后合入」：质量审查报告一处「主要」问题（P-1），其余为次要与细节。由独立子代理修复（提交 `2e732d2`，由 worktree 中的 `ee7a333` 拣选而来）。第 1 条先写失败的用例（编译失败 `undefined: errMissingData`；只声明哨兵时 `TestScanHeadersMissingData` 的 7 例与 `TestWatcherSentCopyWithoutData` 的 2 例全部失败）；其余各条先确认新用例能拦下审查中存活的变异（规格审查的 M13、M15、M29、M37，质量审查的 M20、M32、M33），每个变异重复 3 次都失败。**独立复查尚未进行**（维护者 2026-09-24 暂停本轮工作，留给下一次会话）。

1. **取不到数据的「已发送」副本被越过（质量 P-1，主要；并入规格 D4 的实现部分）：** `Scan` 把「UID SEARCH 列出、FETCH 却没有返回数据」的邮件当作在两条命令之间被删除、越过它并推进游标，`ScanHeaders` 沿用了这一点，Watcher 随后调用 `Scanned(FolderSent)`。服务器对 `UID FETCH (UID RFC822.SIZE)` 或 `BODY.PEEK[HEADER]<0.N>` 只回 OK 而不带数据项、回 `BODY[HEADER] NIL`，或回了别的节（规格 D4：回 `BODY[]` 时，大于 64 KiB 的副本被判为 `TooLarge`，Task 10 读不出其中的标识，只能忽略）时，副本被越过、通知永远记不下实际投递 ID，10 分钟后对它的合法回复被 `thread_mismatch` 永久拒绝，而游标已经越过，此后每一轮都「成功」。修法：只在 `ScanHeaders` 中，这三类情形返回包内哨兵 `errMissingData`（不是也不包装 `ErrClosed`，文本固定，不含文件夹名与 UID），本批作废、游标不前进，连接仍可用。这是对契约「与 `Scan` 相同」字面的有意偏差，理由是 D6 的证据必须真实。判定在 FETCH 命令完成之后进行：在命令进行中返回错误，`do` 会把它当作连接层失败而关闭连接。节按响应解析（已核对 imapclient 的 `readSectionSpec`：`HEADER.FIELDS` 同样被记为 `Specifier` HEADER），所以不只比较 `Specifier`：整封 `BODY[]`、某个部分的头部 `BODY[1.HEADER]`、只含部分字段的头部、起始偏移不为 0 的部分取回都拒绝；`BODY[HEADER]<0>` 与不带偏移的 `BODY[HEADER]`（服务器不遵守部分取回）照常接受。`Scan`（INBOX 与 Junk）不变，仍按已删除越过，遇到不一致的节也照旧接受（修正前即如此，不是想要的行为，只是没改，没有用例钉住）。Watcher 不用改：`drainSent` 发出 `folder_unavailable`、不调用 `Scanned`、不置 `resumeInbox`，下一轮重试；真被删除的邮件下一次 UID SEARCH 不再列出，补扫自行恢复。测试：假服务器新增 `faultReply`（原样写出一条无标签响应，再回 `<标签> OK`）；新增 `TestScanHeadersMissingData`（7 例：两种不带数据、NIL、四种节不一致；断言返回 `errMissingData` 且不是 `ErrClosed`，紧接着在同一连接上再补扫即交付这封邮件的头部）；新增 `TestWatcherSentCopyWithoutData`（2 例：失败的那一轮发出 `folder_unavailable`、不调用 `Scanned(FolderSent)`、游标不动，唤醒后的下一轮仍以「已发送」开头、先交付副本再调用 `Scanned`、不重新登录）；`TestScanSkipsVanishedMessages` 补 NIL 正文一例（`Scan` 仍越过）。变异：三处检查各自去掉都被拦下；前两处施加到 `Scan` 被 `TestScanSkipsVanishedMessages` 拦下，节的检查施加到 `Scan` 被 `TestScanFetchesNewMessagesReadOnly` 拦下；错误包装 `ErrClosed` 而连接其实没关，只能由 `TestScanHeadersMissingData` 拦下（Watcher 按 `s.closed` 判断 `resumeInbox`，这个变异在 Watcher 层没有可观察的差别）；在 FETCH 命令内返回这个错误、使连接被关闭，两个用例都拦下。
2. **「已发送」失败后下一轮照常重试（规格 S1、质量 S-1，次要）：** `r.resumeInbox = s.closed` 改成 `= true`（M15、M20）存活：`TestWatcherSentFailuresNeverDegrade` 的带标签 BAD 与 `NO [UNAVAILABLE]` 两例改为断言每一轮都以「已发送」的 EXAMINE 开头（`assertRoundsStartWithSent`）。只在 `ErrTimeout` 时置 `resumeInbox`（M32）存活：新增「server disconnects」一例，与无标签 BAD 一例共用断言（每条新连接的第一条 EXAMINE 是 INBOX，每次失败一条 `folder_unavailable`，不调用 `Scanned(FolderSent)`，不降级）。
3. **唤醒不缩短登录前的等待（规格 S2、质量 S-2，次要）：** M29、M33 存活。`TestWatcherAuthFailure` 新增「rejected while wakes keep arriving」（每 5 毫秒唤醒一次，暂停期间的 LOGIN 次数、Delay 与登录间隔断言照常成立）；新增 `TestWatcherWakeKeepsReconnectBackoff`（服务器每次收到 LOGIN 都断开，持续唤醒下两次登录的间隔仍不小于其间 backoff 状态中的等待，已含抖动，另留 10% 余量）。`Wake` 与 `Run` 的注释写明唤醒只结束「等待新邮件」。
4. **等待前已有的唤醒信号不复位退避（规格 S3，次要）：** M13 存活。新增 `TestWatcherPendingWakeIsNotIdle`，另断言每条连接上两次 EXAMINE INBOX 之后才出现 IDLE，防止唤醒信号没有排队时空转通过。
5. **跳过历史的那一轮也调用 `Scanned`（规格 D1，细节）：** M37 存活。`TestWatcherScanned` 新增「on the round that skips history」一例，并断言「已发送」没有交付任何邮件。
6. **只认 `Scanned(FolderSent)` 为证据（规格 S4，跨任务）：** 已写入 Task 10 的测试清单（`8f22c01`）。
7. **architecture.md（规格 D2，细节）：** 改为「失败的一轮跳过「已发送」（失败拆掉了连接的，重连后从 INBOX 继续），下一轮重试；不像 Junk 那样在三次连续失败后跳过一小时，「已发送」从不降级」，并补上「列出却取不到头部的邮件使本轮失败」；与 `junkFailLimit`、`relistInterval` 核对一致。
8. **唤醒通道的两条约定（规格 D3、质量 D-1，细节）：** `Watcher.Wake` 的注释写明通道不得关闭（关闭的通道永远就绪，实测 300 毫秒内 1217 条 EXAMINE、0 条 IDLE），`ErrDefer` 之后的唤醒必须延时发出；Task 13 的部分已写入其契约与测试（`8f22c01`）。质量 D-1 建议的运行时防护（把关闭的通道置 nil）不采纳：用转发协程实现会改变「已有信号不算 IDLE 正常结束」的语义。
9. **`Scanned` 的例外（质量 S-3）：** 注释补上：SkipHistory 返回真的那一轮没有交付 UIDNEXT 之前的邮件，仍然调用 `Scanned`，所以 SkipHistory 只有在这些邮件不可能是任何已有通知的副本时才能返回真，查询失败时必须返回假；`ScanHeaders` 把取不到数据的副本当作本轮失败，所以 `Scanned(FolderSent)` 不会越过它们。
10. **包注释言过其实（质量 D-2，细节）：** 「不影响 INBOX」改为「不会让 INBOX 饿死」，并写明：「已发送」的超时或断开拆掉连接时，本轮 INBOX 要等重连之后才补扫；它的本地处理失败时，INBOX 要先等它在同一连接内的重试结束，且共用的本地重试退避不复位（任何一次补扫成功都会复位）。
11. **登录上限（质量 D-3，细节）：** `drainSent` 的注释补一句：「已发送」持续超时时每轮都要重连一次，收取循环会长期顶在 `MaxLogins` 上限附近，这是「从不降级」的直接代价，仍在 4a 的上限之内；「风险与后续」已记（`8f22c01`）。
12. **L2 验证（规格 D4 的 L2 部分）：** 已写入 Task 16 与「风险与后续」（`8f22c01`）。
13. **不采纳：** 规格 D5（`ErrDefer` 时复位 `r.local`）。契约要求的「不退避」已经满足；保留已加倍的基准只影响此后本地失败的重试间隔，偏保守。

验证：`make check` 通过（本包 `-race -covermode=atomic` 覆盖率两次分别为 98.1% 与 97.4%，差别来自取决于时序的分支；总覆盖率 94.35% 与 94.27%）；`go test -race -count=2 ./internal/mail/imap/`、新增与改动的用例 `-count=5` 与 `-race -count=3`、`CGO_ENABLED=0 go test ./internal/mail/imap/`、`make secrets`、两个平台的 vet、不可见字符检查通过；本节共做变异 14 个，全部被拦下。未运行：govulncheck（云端网络策略拒绝 vuln.go.dev）。
...
**审查发现（2026-09-24，待处理）：** 规格审查已完成，结论「修复后合入」：没有主要问题，1 条次要、6 条细节；契约「测试」段每一项都有用例且断言的是契约要求的错误分类，行为与 L1 实测一致，接口能支撑 Task 9–15 的全部用法（审查者用产品的 Watcher 实测驱动了模拟器）。实现者第 3 节的 14 条解释逐条核实均成立（`AllowInsecureAuth` 不造成明文入口：每个连接先包 `tls.Server`、不公告 STARTTLS；样本字节不变的声明经独立组装核对属实）。质量审查也已完成（结论同样是「修复后合入」：1 条主要、4 条次要、3 条细节，见下方第 9–16 条）。**修复与独立复查尚未进行**（维护者 2026-09-24 暂停本轮工作）。
...
质量审查的发现（机密方面没有问题：授权码在运行时构造，错误文本都是静态文字，两个服务端的调试输出都关着；并发压力测试 `-race` 跑 3 次没有数据竞争与 panic；实现者 14 条解释逐条成立）：

9. **M1 IDLE 会丢失唤醒（主要）：** `imapSession` 没有覆盖 `Idle`，imapmemserver 的 `SessionTracker.Idle` 只等待开始之后到达的更新，不先写出已排队的更新，而 imapserver 只在命令结束时 Poll、SELECT/EXAMINE 不 Poll。所以上一条命令的 OK 之后、IDLE 登记之前追加的邮件，要等到 IdleMax 或下一条命令才报告——产品 Watcher 每轮最后正是 `Examine(INBOX)` 接着 IDLE。复现：`Examine(INBOX)` → `Deliver(INBOX)` → `Idle`（IdleMax 2 秒），2 秒后返回 false。Task 13、14 的回环刹车要跑 11 轮与 41 轮，会成为间歇超时的来源；`TestDeliverWakesIdle` 也只靠 50 毫秒的提前量通过。修法（审查者已验证）：每次追加后关闭并换新一个 `changed` 通道；`imapSession.Idle` 循环执行「取当前的 `changed` → `UserSession.Poll(w, true)` → 等 `changed` 或 stop」，修后同一复现 1 毫秒返回 true；把复现补成确定性用例。
10. **S1 追加时持有 `s.mu` 去等 imapmemserver 的邮箱锁（次要）：** `Deliver`、SMTP 收下时写「已发送」、`release` 都在持有 `s.mu` 时调用 imapmemserver 的追加，而它的 `MailboxView.Fetch` 在整个 FETCH（含网络写出）期间持有该文件夹的锁。客户端停止读取时，`Stats`、`SetFaults`、`Messages`、`DisconnectAll`、`Close` 与每个 SMTP 步骤都会阻塞，`Close` 放在 `t.Cleanup` 里会挂住整个测试二进制；「已发送」上的 FETCH 停住时 `smtp.Send` 还会得到假的 `ErrUncertain`。只在测试中有停止读取的原始客户端时触发。修法（已验证）：用单独的 `appendMu` 串行化追加（保持 `folders` 与 UID 一致），`s.mu` 只保护内存状态、持有它时不调用 imapmemserver，`Close` 先 `DisconnectAll` 再关两个服务；「已发送」上的 FETCH 停住时 `Send` 仍要等待，是 imapmemserver 每个文件夹一把锁的上游限制，写进注释。
11. **S2 FETCH 写字面量途中断开时 imapserver 的连接协程永久泄漏（次要，上游缺陷）：** imapmemserver 的 `message.fetch` 写出错时直接返回、不调用 `w.Close()`，响应编码器的锁不释放，其后 `readCommand` → `writeStatusResp` 永久阻塞；泄漏的协程留住连接、会话、该文件夹的 SessionTracker 与整个 `*Server`。`DisconnectAll`（Task 14 的断网恢复）、`Close` 或产品客户端在 FETCH 中途超时都会触发，只在字面量大于约 4 KiB 时发生。修法（待确认）：覆盖 `imapSession.Fetch`，`UserSession.Fetch` 返回错误（只可能是写连接失败）时 panic 一个包内哨兵，由 imapserver 的 `serve` recover 并关闭连接与会话（修后 5 毫秒内服务端协程归零）；不接受以 panic 控制流程的话，至少在 `Close`、`DisconnectAll` 的注释里写明，提醒后续任务不要对模拟器做协程泄漏断言，并考虑报告上游。
12. **S3 BDAT（CHUNKING）会让会话挂住（次要）：** go-smtp 总是公告 PIPELINING 与 CHUNKING；`smtpConn` 在命令模式下按行读取，块数据不以 LF 结尾时双方死锁（复现：`BDAT 5 LAST\r\nHello` 之后 3 秒没有响应）；BDAT 时 go-smtp 在另一个协程里调用 `Session.Data`，「与 Session 的方法在同一协程中依次执行」的注释不成立（目前没有数据竞争）。产品客户端只用 DATA，不受影响。修法（已验证）：命令模式看到 `BDAT <n> [LAST]` 时记下 n、恰好转发 n 字节，这一判断优先于数据模式与按行模式，并改正两处注释。
13. **S4 测试缺口：** 审查者自拟 20 个变异，15 个存活，都有在原代码上通过、在变异上失败的补测草稿：`Delivered` 的顺序、`readLine` 的 `maxCommandLine` 上限、DATA 故障的大小写与只有 LF 的行尾、`Deliver` 复制调用方的切片、STATUS 与 `Messages(Sent)` 写入到期副本、授权码拒绝空格、多个收件人、`Reset` 清空收件人（同一连接发两封）、收件人副本与延迟副本不共用内存、`Drop` 优先于 `SubmissionCode`、续行的制表符、INTERNALDATE 取注入时钟、mailfixture 中 lf 部件的多行取值以 LF 连接。
14. **D1–D3（细节）：** `Close` 已置 closed、还没断开连接时完成的 DATA 会回 250 而邮件不进「已发送」也不进 `Delivered`（建议 `accept` 返回是否收下，没收下时回 421）；DATA 故障只拦截恰为 `DATA` 的一行，两个以上空格与管线化中紧随结束标记的命令会绕过（产品客户端不受影响，比较前 TrimSpace 并在注释中写明管线化的限制）；设了 `SentExamineFails` 时 STATUS「已发送」照常成功（契约只提到 EXAMINE，产品目前也不发 STATUS；若 Task 15 改用 STATUS 取 UIDNEXT，在 `Faults` 的注释里点明）。
15. **范围外（Task 6 的 Watcher，已记为 Task 6 实施说明中的既有问题）：** 补扫 Junk 期间到达 INBOX 的邮件要等到 IdleMax（生产 5 分钟）：imapclient 把 SELECT 响应里的 EXISTS 记进 SELECT 的结果，Watcher 也不比较 `Examine(INBOX)` 返回的邮件数。修 M1 消除不了它，Task 14 的端到端测试仍需要较短的 IdleMax；是否让 Watcher 比较邮件数，恢复工作时再定。
16. **给后续任务的提醒：** `internal/mail/imap` 与 `internal/mail/smtp` 的包内测试不能导入 qqsim（会循环导入）；审查者的修复草稿 A–D 与补测草稿只存在于本次会话的临时目录，恢复工作时按上面的描述重做。
