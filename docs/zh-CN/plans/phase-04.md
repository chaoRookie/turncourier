...
| `TestL1Idle` | 在 INBOX 上连续 IDLE，时长 `TURNCOURIER_LIVE_IDLE_MINUTES`（默认 30，至多 60），`IdleMax` 同时放宽到该时长；记录每次 EXISTS 推送的时刻与服务器断开连接的时刻。进入 IDLE 之前先做预检：`Examine(INBOX)` 记下邮件数与 UIDNEXT，以返回的 UIDVALIDITY 与 UIDNEXT−1 构造游标（不取回已有邮件），以该游标连续 `Scan(INBOX)` 两次，再 `Examine(INBOX)` 一次，随后第一次调用 `Idle`，记录其返回值与耗时。EXAMINE 响应中的 EXISTS 归入命令结果，不留下信号；因此若第一次 `Idle` 不发 IDLE 就返回 true（耗时只是一次本地通道检查，远小于 IDLE 与 DONE 两次往返），且前后两次 EXAMINE 的邮件数与 UIDNEXT 都不变，即说明服务器在邮件数不变时也在 UID SEARCH 的响应中附带 `* N EXISTS`。这次返回单独记录，不计入 EXISTS 推送时刻。除下述含 UID FETCH 的检查外，各次 `Idle` 之间不调用 `Scan`，其余返回都来自 IDLE 期间的推送。设置 `TURNCOURIER_LIVE_IDLE_SELF_SEND=1` 时，预检之后发一封确认过的邮件给机器人自己，记录从 250 响应到 EXISTS 的延迟；随后做含 UID FETCH 的检查：`Examine(INBOX)`，以预检的游标 `Scan(INBOX)` 一次（取回这封自发邮件，不输出其内容），再 `Examine(INBOX)`，随后调用 `Idle` 并同样单独记录返回值与耗时；两次 EXAMINE 之间邮件数与 UIDNEXT 不变而 `Idle` 立即返回，说明 UID SEARCH 或 UID FETCH 的响应附带 EXISTS，与预检结果对照即可分辨 | 可选，逐封确认 |
...
| 4 | `-run TestL1Idle`；可加 `TURNCOURIER_LIVE_IDLE_SELF_SEND=1` | 结果用于确认或调整 Task 11 的 `IdleMax`、退避与 `Poll` 数值。邮件数不变时 UID SEARCH 的响应是否带 EXISTS，由 `TestL1Idle` 的预检记录；UID FETCH 的检查需加 `TURNCOURIER_LIVE_IDLE_SELF_SEND=1`；两次检查中的立即返回都不计入推送统计 |
