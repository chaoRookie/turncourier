...
- Sending or receiving email in the product. The SMTP and IMAP clients exist, but no command calls them, and nothing renders a notification or parses an inbound message. MIME parsing, quote and signature parsing, reply validation, the thread binding rules and the deterministic filtering of outgoing content are all Phase 4b, after L1 has answered how QQ Mail rewrites Message-IDs and how each client formats a reply.
...
| `github.com/emersion/go-sasl` | `b788ff22d5a6` | MIT | Pulled in by go-imap/v2; not used directly. |
| `golang.org/x/text` | v0.42.0 | BSD-3-Clause | Only `go-message/charset` imports it, so it enters the build graph through `tests/live`. Pinned explicitly: the v0.14.0 that go-message asks for carries GO-2026-5970, fixed in v0.39.0. |

`modernc.org/sqlite` also brings in `dustin/go-humanize`, `google/uuid`, `mattn/go-isatty`, `ncruces/go-strftime`, `remyoudompheng/bigfft`, `golang.org/x/sys`, `modernc.org/libc`, `modernc.org/mathutil` and `modernc.org/memory`, all under MIT or BSD-style licenses. `golang.org/x/sys` is pinned at v0.48.0, which `golang.org/x/term` v0.46.0 requires.
