...
internal/mail/renderer       ─► internal/mail, go-message (mail)
                                (takes the subject tag through its own interface)
...
`internal/mail` and its subpackages `smtp`, `imap`, `parser` and `renderer` depend on neither storage, configuration nor `internal/security/*`. They move, parse and render bytes; cursors and passwords arrive as values or as functions, and the parser and the renderer take the footer constants from `internal/mail` rather than from the token package. The renderer gets the subject tag through its own `Tag` interface, which `token.Tag` satisfies, and the process secrets through a `Scrubber` interface. The three `internal/security` packages do not import each other. At the end of Phase 4a no production package imports both a mail package and storage; `internal/app`, which will, is Phase 4b work. Outside the product binary, `tests/live` is the second assembler.
...
| `internal/mail/renderer` | Renders outgoing notifications. `Content` is the notification content stored encrypted with the notification row: JSON `{"v":1,"event":…,"task":…,"title":…,"body":…}`, at most 1 MiB, decoded strictly (exact field names, no unknown, repeated, missing or null fields, the literal version 1, nothing after the object), and redacted in `String`, `Format`, `LogValue` and `MarshalJSON`. `Filter` is the deterministic content filter, applied in a fixed order: control and Unicode format characters; fenced code blocks, replaced by a line count, and then private key blocks (both span lines, so they go before the per-line rules); process secrets through a `Scrubber` and token-shaped strings; URL userinfo, query strings and fragments (`file://` links count as paths); absolute paths outside links; `Authorization` values, values of keys whose names end in `password`, `secret`, `token`, `api_key` and the like, and strings with well-known token prefixes, none of which may touch a link's scheme; and finally truncation to 4000 characters at a cut that the earlier steps leave unchanged. It is idempotent and does not promise to find every secret. `Render` writes the subject as the raw ASCII tag followed by the B-encoded title (event name plus the filtered title, at most 60 characters; the event name only in status mode), so the tag never enters an encoded-word or gets folded. It adds `In-Reply-To` and `References`, `Auto-Submitted: auto-generated` and `X-TurnCourier-ID`, and a `multipart/alternative` body with a plain text part and an HTML part holding the same text escaped inside a `pre-wrap` container, both UTF-8 and quoted-printable, followed by the footer: marker line, task and event, reply instruction, token copy, omission note and two reminders. Message-IDs and addresses are checked for shape before any header is written; results over 256 KiB are refused; errors never echo content, tokens or addresses. |
...
| `github.com/emersion/go-message` | v0.18.2 | MIT | `internal/mail/parser` reads the header and MIME structure of inbound mail with it and decodes charsets through `charset`, where it maps the GBK labels to GB18030. `internal/mail/renderer` assembles outgoing notifications with it (`mail` headers, the `multipart/alternative` writer and quoted-printable parts). It also reaches `internal/mail/imap` because `imapclient` imports `go-message/mail`. `tests/live` imports it directly, with `charset` for GB18030 and GBK decoding. |
