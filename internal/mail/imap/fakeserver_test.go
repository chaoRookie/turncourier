...
// 模拟 QQ 的无标签 BAD、半开连接、问候冻结、文件夹暂时不可用、补扫中到达的 EXISTS、少报的邮件大小、断开、能力差异与明文入口。
...
type rule struct {
	...
	kind     faultKind // 故障种类
	bytes    int       // faultFreezeAfter 转发的响应字节数
...
func (fs *fakeServer) serve(ln net.Listener, backend string, serverTLS *tls.Config) {
	for {
		...
		fs.accepted++
		fs.mu.Unlock()
		go fs.handle(conn, backend, serverTLS)
	}
}

// handle 完成 TLS 握手（明文入口除外），连接 imapmemserver，并启动两个方向的转发。
func (fs *fakeServer) handle(raw net.Conn, backend string, serverTLS *tls.Config) {
	client := raw
