...
- [x] 4b Task 7（QQ 行为模拟器 `tests/qqsim`）：实现已合入（`ca9d412`），29 个变异全部被拦下；规格审查与质量审查都已完成（都是「修复后合入」，记在 phase-04.md Task 7 之下），修复与独立复查未做。
...
- [ ] **4b 实现（暂停中）**：恢复时按这个顺序处理已有的积压，再往下推进——
  1. Task 7：规格与质量审查都已完成，发现记在 phase-04.md Task 7 之下（主要 1 条：模拟器的 IDLE 会丢失在 EXAMINE 与 IDLE 之间到达的邮件，Task 13、14 的长测试会因此间歇超时；次要 5 条：文件夹名须以 L1 实测的字面量钉住、追加时持锁等 imapmemserver 的邮箱锁、FETCH 中途断开的协程泄漏、BDAT 死锁、15 个存活变异的补测）→ 修复 → 独立复查。
